...
import {
  ...
  Clock, Mic, Star, Crown, Sparkles, ArrowRight, BarChart3, Gift, Copy, Check
} from "lucide-react";
import { useAuth, REFERRAL_GOAL } from "../../../context/AuthContext";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (user) setReferralLink(`${window.location.origin}/${lang}/login?ref=${user.uid}`);
  }, [user, lang]);
    ...
    <div className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      <div className="container mx-auto px-4 sm:px-6 max-w-6xl py-8">
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-primary/5 to-transparent border border-primary/20 rounded-2xl p-6 mb-8"
        >
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="font-bold text-slate-900 dark:text-white mb-1">
                  Invite 3 friends, unlock 30 days Premium — free
                </h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">
                  Share your link below. Every friend who signs up counts toward your unlock.
                </p>
                <div className="flex items-center gap-2">
                  {Array.from({ length: REFERRAL_GOAL }).map((_, i) => {
                    const filled = i < ((userData?.referralCount || 0) % REFERRAL_GOAL);
                    return (
                      <div
                        key={i}
                        className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                          filled ? "bg-primary text-white" : "bg-slate-100 dark:bg-white/[0.06] text-slate-400"
                        }`}
                      >
                        {filled ? <Check className="w-3.5 h-3.5" /> : i + 1}
                      </div>
                    );
                  })}
                  <span className="text-xs text-slate-500 dark:text-slate-400 ml-1">
                    {(userData?.referralCount || 0) % REFERRAL_GOAL}/{REFERRAL_GOAL} invited
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              <input
                ...
                className="form-input text-xs flex-1 md:w-64 truncate"
              />
              <button
